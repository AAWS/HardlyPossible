...
public class myWorld extends Environment {
	...
	private static final long serialVersionUID = -7325344005640129740L;
	boolean inLevelView = true;
	private static myWorld instance;
	int levelFactor = 3;
	public int scrollPosition;
	public static List<myPaintable> inPaintable = new ArrayList<myPaintable>();
	public static List<myScrollable> inScrolling = new ArrayList<myScrollable>();
	public static List<myActable> inActing = new ArrayList<myActable>();
	public static List<myIntersectable> inIntersectable = new ArrayList<myIntersectable>();
	private static Set<String> keysDown = new HashSet<String>();
	...
	public void setBackground(int background) {
		...
		if (bg == null) { return; }
		bg = bg.getScaledInstance(java.awt.Toolkit.getDefaultToolkit().getScreenSize().width, java.awt.Toolkit.getDefaultToolkit()
				.getScreenSize().height, 1);
		setBackground(bg);
	...
	static String getKeyText(int keyCode) {
		if (keyCode >= KeyEvent.VK_0 && keyCode <= KeyEvent.VK_9 || keyCode >= KeyEvent.VK_A && keyCode <= KeyEvent.VK_Z) { return String
				.valueOf((char) keyCode); }
		switch (keyCode) {
	...
	public void paintEnvironment(Graphics graphics) {
		try {
			for (myPaintable p : inPaintable) {
				if (p.getX() + p.getWidth() > 0 && p.getX() + p.getWidth() < screenSize.width && p.getY() + p.getHeight() > 0
						&& p.getY() + p.getHeight() < screenSize.height) {
					if (!inLevelView) {
						BufferedImage img = p.paint();
						graphics.drawImage(img.getScaledInstance(img.getWidth() / levelFactor, img.getHeight() / levelFactor, 1),
	...
	public class JSound extends JPlayer.PlaybackListener implements Runnable {
		...
		public JSound(int level) {
			set(level);
			this.filePath = "resources/sounds/" + slug + ".mp3";
		}
		...
		private void playerInitialize() {
			try {
				this.player = new JPlayer(filePath, this);
